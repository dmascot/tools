...
function Install-NVM {
    ...
    if(-not(isCommandInstalled -commandName "nvm" )) {
        ...
        $NVM_PATH = "/coreybutler/nvm-windows/releases/latest"

        $URI =  [Uri]::new([Uri]::new($BASE_URL), $NVM_PATH).ToString()

        $Req = Invoke-WebRequest -UseBasicParsing  -Uri $URI

        $Download_Part = ($Req.Links | Where-Object outerhtml -match "nvm-setup.exe").href

        $DownloadLink =  [Uri]::new([Uri]::new($BASE_URL), $Download_Part).ToString()
