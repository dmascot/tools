...
test_is_running_as_root_with_non_root(){
    ...
    then
        ...
        result=$(sudo -u $NEW_USER bash -c 'source scripts/linux/functions.sh; is_running_as_root')
    fi
...
test_is_running_as_root_with_sudo(){
    ...
    if is_running_as_root; then result=$(sudo -u root bash -c 'source scripts/linux/functions.sh; is_running_as_root'); fi 
